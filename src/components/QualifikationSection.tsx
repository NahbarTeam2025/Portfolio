...
export const QualifikationSection = React.memo(({ 
  ...
  return (
    <div className="flex flex-col items-start gap-4 md:gap-8 w-full animate-in fade-in duration-500">
      ...
      <div className={`relative flex flex-col gap-6 w-full max-w-[1200px] pb-12 transition-all duration-500 ${expandedQual === null ? 'pl-8 md:pl-16' : 'pl-0'}`}>
        ...
        {showTimeline && (
          <motion.div 
            ...
            className="absolute left-[15px] md:left-[31px] top-4 bottom-16 w-[1px] bg-gradient-to-b from-brand-teal via-brand-violet to-brand-teal" 
          />
        )}
            ...
            >
              ...
              {showTimeline && (
                <motion.div 
                  ...
                  className="absolute -left-[23px] md:-left-[41px] top-[38px] w-3 h-3 rounded-full bg-black border border-brand-teal z-20 shadow-[0_0_10px_rgba(5,184,194,0.5)] group-hover:scale-125 transition-transform duration-300" 
                />
              )}
                    ...
                    {qual.items && (
                      <div className="flex flex-wrap gap-2">
                        {qual.items.map((item, j) => (
                          <span 
                            key={j} 
                            className={`px-3 py-1 rounded-full border text-[11px] md:text-[12px] font-medium transition-all duration-300 hover:scale-105 hover:bg-white/5 ${
                              j % 2 === 0 
