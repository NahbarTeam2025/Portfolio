...
export const UberMichSection = React.memo(({ handleNavigate }: { handleNavigate: (page: string) => void }) => {
  ...
  return (
      <div className="flex flex-col items-start gap-1 md:gap-1.5 lg:gap-2 w-full flex-grow animate-in fade-in duration-500 pb-1 md:pb-1">
        ...
        <div className="flex flex-col gap-1 md:gap-1.5 text-white/90 text-[15px] sm:text-[17px] md:text-[19px] lg:text-[21px] font-normal max-w-2xl leading-relaxed pt-0.5 md:pt-2">
          <p className="font-bold text-white text-[16px] sm:text-[19px] md:text-[23px] lg:text-[26px] mb-0.5 md:mb-1.5">
            {t.about.boldText}
          </p>
          ...
          <div className="pt-2 md:pt-3 flex flex-col gap-1 md:gap-2">
            <p className="text-white font-medium tracking-wider text-[13px] sm:text-[15px] md:text-[17px] lg:text-[19px] leading-relaxed">
              {t.about.motto}
            </p>
                ...
                return (
                  <div key={i} className="relative group" ref={(el) => (cardRefs.current[i] = el)}>
                    ...
                    <motion.div 
                      ...
                      className="absolute -left-[16px] md:-left-[24px] -translate-x-1/2 top-6 w-3 h-3 md:w-4 md:h-4 rounded-full bg-black border-2 border-blue-500/60 z-10 group-hover:scale-125 group-hover:border-blue-400 transition-all duration-300"
                    >
                      <div className="absolute inset-0 rounded-full bg-blue-500/20 animate-ping" />
                    </motion.div>

                    <div className={`wow-card flex flex-col transition-all duration-500 ${isExpanded ? 'ring-1 ring-gray-400/30' : 'overflow-hidden'}`}>
                          ...
                          {qual.content && (
                            <div className="flex flex-wrap gap-2">
                              {qual.content.map((item: string, j: number) => (
                                <span key={j} className="px-2.5 py-1 rounded-full border border-gray-400/40 text-[12px] md:text-[13px] font-medium text-white">
                                  {item}
                                </span>
                              ))}
                            </div>
