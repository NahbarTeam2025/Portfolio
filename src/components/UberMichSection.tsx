                      ...
                      >
                        <div className="flex flex-col gap-0.5">
                          <div className="flex items-center gap-3">
                            <h3 className="text-[15px] md:text-base font-semibold text-black leading-tight group-hover/btn:text-blue-400 transition-colors">{qual.title}</h3>
                            <span className="hidden sm:inline-block text-gray-800 font-mono text-[10px] bg-gray-600/20 border border-gray-600/30 px-2 py-0.5 rounded-md">{qual.date}</span>
                          </div>
                          <span className="text-black/80 text-[13px] md:text-[14px]">{qual.company}</span>
                        </div>
                        <div className={`w-7 h-7 rounded-full border border-black/20 flex items-center justify-center transition-transform duration-300 ${isExpanded ? 'rotate-180 bg-brand-blue/20 border-brand-blue/40' : ''}`}>
                      ...
                      <div className={`transition-all duration-500 ease-in-out relative z-10 ${isExpanded ? 'max-h-[none] opacity-100' : 'max-h-0 opacity-0 overflow-hidden'}`}>
                        <div className="px-4 pb-4 md:px-5 md:pb-5 flex flex-col gap-3">
                          ...
                          <div className="sm:hidden mb-1">
                            <span className="text-gray-800 font-mono text-[10px] bg-gray-600/20 border border-gray-600/30 px-2 py-0.5 rounded-md">{qual.date}</span>
                          </div>
                          {qual.desc && (
                            <div className="text-black/70 text-[14px] md:text-[15px] leading-relaxed [&_strong]:text-black [&_strong]:font-bold [&_p]:mb-4">
                          ...
                          {qual.content && (
                            <div className="flex flex-wrap gap-2">
                              {qual.content.map((item: string, j: number) => (
                                <span key={j} className="px-2.5 py-1 rounded-full border border-gray-600/40 text-[12px] md:text-[13px] font-medium text-black">
                                  {item}
                                </span>
                              ))}
                            </div>
