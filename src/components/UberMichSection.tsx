...
export const UberMichSection = React.memo(({ step, setStep }: any) => {
  return (
    <div className="flex flex-col items-start gap-4 md:gap-8 w-full h-full overflow-hidden animate-in fade-in duration-500">
      <h1 className="heading-gradient text-[24px] md:text-[40px] lg:text-[56px] font-medium leading-[1.2] tracking-tight shrink-0">
        Über mich
      </h1>
      <div className="w-full h-[1px] bg-white/10 shrink-0" />
      <div className="flex flex-col gap-3 md:gap-6 text-white/80 text-[13px] md:text-[16px] lg:text-[20px] font-normal max-w-[1000px] leading-relaxed pt-2 md:pt-8 overflow-y-auto md:overflow-visible">
        ...
        <p className="relative text-brand-teal font-medium tracking-wider text-[11px] md:text-[14px] lg:text-[16px] uppercase">
          <span className="opacity-0 select-none pointer-events-none block" aria-hidden="true">
        ...
        <p className="relative text-white font-medium mt-1 md:mt-4 text-[14px] md:text-[18px] lg:text-[24px]">
          <span className="opacity-0 select-none pointer-events-none block" aria-hidden="true">
        ...
        <div className={`mt-2 md:mt-8 transition-opacity duration-1000 ${step >= 4 ? 'opacity-100' : 'opacity-0'}`}>
          <img 
            src="https://ik.imagekit.io/roberterbach/site-signature.png?tr=w-400" 
            alt="Unterschrift Robert Erbach" 
            className="h-10 md:h-20 w-auto object-contain invert mix-blend-screen opacity-90"
            loading="lazy"
            decoding="async"
          />
        </div>
