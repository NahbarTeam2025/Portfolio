        ...
        <div className="space-y-3 bg-white/5 p-4 rounded-2xl border border-white/10 backdrop-blur-sm">
          <div className="relative">
            ...
            <div className="absolute left-4 top-1/2 -translate-y-1/2">
              {strength >= 3 ? <Lock className="w-5 h-5 text-blue-400" /> : <Unlock className="w-5 h-5 text-white/20" />}
            </div>
            <button
          ...
          <p className="text-[8px] text-white/60 text-center uppercase tracking-widest">
            {t.tools.noDataStored}
          </p>

          {/* Strength Meter */}
          <div className="space-y-3">
            <div className="flex justify-between text-[10px] uppercase tracking-[0.2em] text-white/60 font-mono">
            ...
            <div className="h-1.5 w-full bg-white/5 rounded-full overflow-hidden flex gap-1">
              {[0, 1, 2, 3].map((i) => (
                <div key={i} className="flex-grow h-full bg-white/5 relative">
                  <motion.div
                    initial={{ scaleX: 0 }}
          ...
          <div className="grid grid-cols-2 gap-2 pt-2">
            {[
              ...
              <div key={req.label} className="flex items-center gap-2">
                <div className={`w-1 h-1 rounded-full ${req.met ? 'bg-blue-400 shadow-[0_0_5px_#3b82f6]' : 'bg-white/10'}`} />
                <span className={`text-[9px] uppercase tracking-wider ${req.met ? 'text-white/80' : 'text-white/30'}`}>{req.label}</span>
              </div>
            ))}
          </div>
        </div>
