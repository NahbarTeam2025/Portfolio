...
export const DatenschutzModal = ({ setIsDatenschutz }: { setIsDatenschutz: (open: boolean) => void }) => {
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 sm:p-6" role="dialog" aria-modal="true" aria-labelledby="modal-title">
      ...
      <div className="relative w-full max-w-3xl max-h-[85vh] max-h-[85dvh] bg-black/20 backdrop-blur-[20px] saturate-[180%] border border-white/10 rounded-2xl shadow-[0_20px_40px_rgba(0,0,0,0.5)] overflow-hidden flex flex-col animate-in fade-in zoom-in-95 duration-300" tabIndex={-1}>
        ...
        <div className="p-6 overflow-y-auto text-white/70 text-sm leading-relaxed space-y-8">
          ...
          <section>
            <h3 className="text-lg font-medium text-white mb-3">{t.privacy.section6.title}</h3>
