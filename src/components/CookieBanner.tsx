...
export const CookieBanner: React.FC<{ handleNavigate: (page: string) => void }> = ({ handleNavigate }) => {
  ...
  return (
    ...
    >
      <div className="max-w-xl w-full bg-black/90 backdrop-blur-xl border border-white/20 rounded-t-xl md:rounded-xl shadow-[0_0_50px_rgba(0,0,0,0.5)] p-4 md:p-5 text-white flex flex-col sm:flex-row items-center justify-between gap-4 md:gap-6 pointer-events-auto ring-1 ring-white/10">
        ...
        <div className="flex items-center gap-3 shrink-0 w-full sm:w-auto">
          <button
            ...
            className="flex-1 sm:flex-none px-6 py-2 rounded-lg bg-blue-600 hover:bg-blue-500 text-white text-[12px] font-bold uppercase tracking-wider transition-all duration-300 shadow-lg shadow-blue-600/20 active:scale-95"
          >
            {t?.common?.cookieBanner?.accept || 'Akzeptieren'}
          </button>
          <button
            onClick={handleDecline}
            className="flex-1 sm:flex-none px-6 py-2 rounded-lg bg-white/5 hover:bg-white/10 border border-white/10 text-white/70 hover:text-white text-[12px] font-bold transition-all duration-300 active:scale-95"
          >
